...
// Sequential sum of array elements
int seqSum(const std::vector<int>& arr) {
    int sum = 0;
    for (int num : arr) {
        sum += num;
    }
...
// Sequential search for a key element
bool seqSearch(const std::vector<int>& arr, int key) {
    for (int num : arr) {
        if (num == key) {
            return true;
        }
...
int main() {
    ...
    bool seqSearchResult = seqSearch(arr, key);
